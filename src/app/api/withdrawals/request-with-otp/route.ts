...
export async function POST(request: Request) {
  try {
    ...
    const withdrawalAmount = Number(verification.amount || 0);
    ...
    const walletAddress = String(profile?.wallet_address || "").trim();

    if (walletAddress.length < 20) {
      return NextResponse.json(
        { error: "Please verify and save your USDT BEP20 wallet address first." },
        { status: 400 }
      );
    }

    const { data: walletRowsData, error: walletError } = await adminClient
    ...
    const { data: walletTransaction, error: transactionError } =
      await adminClient
        ...
        .insert({
          ...
          direction: "debit",
          status: "pending",
          description: "Withdrawal request pending founder approval",
