...
function loadQuizFile(courseSlug: string, challengeSlug: string): QuizFile {
  if (!SAFE_SLUG.test(courseSlug) || !SAFE_SLUG.test(challengeSlug)) {
    throw new Error("Invalid Academy quiz path.");
  }
  ...
  if (!fs.existsSync(quizPath)) {
    throw new Error("Quiz content file was not found.");
  }

  const rawQuiz = fs.readFileSync(quizPath, "utf8");
  const parsedQuiz: unknown = JSON.parse(rawQuiz);

  if (!isValidQuizFile(parsedQuiz)) {
    throw new Error("Quiz content file has an invalid structure.");
  }
...
async function loadQuizContext(
  ...
  if (!SAFE_SLUG.test(courseSlug) || !SAFE_SLUG.test(challengeSlug)) {
    return {
      errorResponse: NextResponse.json(
        { error: "The requested quiz path is invalid." },
        { status: 400 }
      ),
    };
  }
  ...
  if (challengeError || !challengeData) {
    return {
      errorResponse: NextResponse.json(
        { error: "This Academy challenge is unavailable." },
        { status: 404 }
      ),
    };
  }

  const challenge = challengeData as ChallengeRecord;
  ...
  const { data: assessmentData, error: assessmentError } = await supabaseAdmin
    ...
    .eq("assessment_type", "challenge_quiz")
    .eq("is_published", true)
    .maybeSingle();
  ...
  if (assessmentError || !assessmentData) {
    return {
      errorResponse: NextResponse.json(
        { error: "The challenge quiz has not been published." },
        { status: 404 }
      ),
    };
  }
  ...
  try {
    quiz = loadQuizFile(courseSlug, challengeSlug);
  } catch (error) {
    console.error("Failed to load Academy quiz file:", error);
      ...
      errorResponse: NextResponse.json(
        {
          error:
            error instanceof Error
              ? error.message
              : "The quiz content could not be loaded.",
        },
        { status: 500 }
      ),
    };
...
export async function GET(request: Request, context: RouteContext) {
  try {
    const { courseSlug, challengeSlug } = await context.params;

    const quizContext = await loadQuizContext(
      request,
      courseSlug,
      challengeSlug
    );

    if ("errorResponse" in quizContext) {
      return quizContext.errorResponse;
    }

    const { course, challenge, assessment, quiz } = quizContext;

    /*
     * Correct answers are deliberately excluded.
     * They must never be sent before quiz submission.
     */
    const safeQuestions = quiz.questions.map((question, index) => ({
      number: index + 1,
    ...
    return NextResponse.json({
      ...
      assessment: {
        ...
        timeLimitMinutes:
          assessment.time_limit_minutes ?? quiz.timeLimitMinutes,
        questionCount: safeQuestions.length,
      },
      questions: safeQuestions,
    });
  } catch (error) {
    console.error("Academy quiz GET error:", error);

    return NextResponse.json(
      { error: "Unexpected server error while loading the quiz." },
      { status: 500 }
    );
  }
}
...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    if (
      ...
      submittedAnswers.length !== quiz.questions.length
    ) {
      return NextResponse.json(
        { error: "Every quiz question must be answered." },
        { status: 400 }
      );
    }
    ...
    if (attemptCountError) {
      return NextResponse.json(
        { error: "We could not verify your previous quiz attempts." },
        { status: 500 }
      );
    }
    ...
    if (
      ...
      return NextResponse.json(
        { error: "You have reached the maximum number of quiz attempts." },
        { status: 403 }
      );
    }
    ...
    if (attemptInsertError || !attemptData) {
      console.error(
        "Failed to save Academy quiz attempt:",
        attemptInsertError
      );
      ...
      return NextResponse.json(
        {
          error:
            ...
            "Your quiz was graded but the result could not be saved.",
        },
        { status: 500 }
      );
    }
    ...
    console.error("Academy quiz POST error:", error);

    return NextResponse.json(
      { error: "Unexpected server error while submitting the quiz." },
      { status: 500 }
    );
  }
}